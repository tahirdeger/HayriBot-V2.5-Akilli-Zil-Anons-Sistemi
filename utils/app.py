...
import shutil
import asyncio
from config.settings import TTS_SETTINGS, media_manager, DB_PATH
...
import re
import sqlite3
from utils.helpers import get_duration, generate_job_id, restart_application
import webbrowser
import utils.tts_manager  # TTS sistemini arka planda başlatmak için


# Medya dizinini belirle (EXE yanındaki klasöre öncelik ver)
if getattr(sys, 'frozen', False):
    base_dir = os.path.dirname(sys.executable)
    local_media = os.path.join(base_dir, "media")
    if os.path.exists(local_media):
        MEDIA_DIR = local_media
        logging.info(f"GUI: Yerel medya dizini kullanılıyor: {MEDIA_DIR}")
    else:
        MEDIA_DIR = str(get_media_path())
else:
    MEDIA_DIR = str(get_media_path())
...
class AyarlarPaneli(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.var_model = tk.StringVar(value=ayar_getir("tts_model", "male"))
        combo_model = ttk.Combobox(frame_ses, textvariable=self.var_model, state="readonly")
        combo_model['values'] = ('ERKEK-Fahrettin', 'KADIN(Yakında)') # male=Fahrettin, female=Yeni Model
...
class ZilYonetimGUI:
    def __init__(self, async_loop):
        ...
        for tur, tree in self.trees.items():
            tree.bind("<Button-3>", self.sag_tik_menu)
        
        self.context_menu = tk.Menu(self.root, tearoff=0)
        self.context_menu.add_command(label="Sil", command=self.zili_sag_tikla_sil)
        self._create_clock()
        self.tray_icon = None
        ...
        # Ses sistemini yenile ve ısıt (3 saniye sonra - Daha hızlı açılış)
        self.root.after(3000, self._warmup_audio_system)
        ...
        def warmup_task():
            try:
                    ...
                    for key, filename in media_files.items():
                        try:
                            ...
                            if os.path.exists(file_path):
                                # Player dictionary'de yoksa ekle
                                if key not in media_manager.players:
                                    media_manager.players[key] = {'path': str(file_path), 'player': None}
                                ...
                                if isinstance(player_data, dict):
                                    ...
                                    if hasattr(media_manager, 'vlc_instance'):
                                        # str() çevrimi önemli!
                                        media = media_manager.vlc_instance.media_new(str(file_path))
                                        new_player = media_manager.vlc_instance.media_player_new()
                                        new_player.set_media(media)
                                        player_data['player'] = new_player
                                        # Path'i de güncelle (önemli!)
                                        player_data['path'] = str(file_path)
                                        
                                        if key == 'startup':
                                            if os.path.getsize(file_path) < 1000:
                                                logging.warning(f"⚠️ Startup dosyası çok küçük ({os.path.getsize(file_path)} bytes). Sessiz olabilir.")
                                            logging.info(f"Startup sesi hazırlandı: {file_path}")
                            else:
                                if key == 'startup':
                                    logging.warning(f"Startup dosyası bulunamadı: {file_path}")
                        except Exception as e:
                ...
                if hasattr(media_manager, 'play_media'):
                    ...
                    time.sleep(0.5) # Beklemeyi kısalttık
                    logging.info("🔊 Başlangıç sesi için sanal buton tetikleniyor...")
                    # Ana thread üzerinden _handle_control çağır (Sanki butona basılmış gibi)
                    self.root.after(0, lambda: self._handle_control('startup'))
    ...
    def _handle_control(self, command):
        ...
        # STARTUP İÇİN İSTİSNA: Startup komutu meşguliyeti yoksayabilir (zaten temizliyor)
        if command != 'startup' and media_manager.is_media_busy():
    ...
    def _create_zil_list_panel(self):
        list_frame = ttk.LabelFrame(
            self.right_panel,
            text="🕒 KAYITLI ZİL SAATLERİ",
            style="Header.TLabelframe"
        )
        list_frame.pack(fill=tk.BOTH, expand=True, pady=1, padx=1)
        
        scroll = ttk.Scrollbar(list_frame)
        scroll.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.numara_tree = ttk.Treeview(
            list_frame,
            columns=("No",),
            show="headings",
            height=10,
        ...
        self.numara_tree.heading("No", text="No")
        self.numara_tree.column("No", width=30, anchor="center")
        self.numara_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        ...
        for tree_id, title in tree_types:
            tree = ttk.Treeview(
                list_frame,
                columns=(tree_id,),
                show="headings",
            ...
            tree.heading(tree_id, text=title)
            tree.column(tree_id, width=70, anchor="center")
        ...
        for i in range(1, 15):
            ...
            self.numara_tree.insert("", tk.END, 
                                  values=(f"{i}.",), 
                                  tags=(tag,))
            for tur, tree in self.trees.items():
                tree.insert("", tk.END, 
        ...
        for tree in [self.numara_tree] + list(self.trees.values()):
            tree.tag_configure('evenrow', background='#e0e0e0')
            tree.tag_configure('oddrow', background='#ffffff')
            tree.tag_configure('selected', background='#bbdefb')
    ...
    def _toggle_selection(self, event, tur):
        ...
        if item:
            ...
            if item in current_selection:
                ...
                original_tag = 'evenrow' if (row_index+1) % 2 == 0 else 'oddrow'
    ...
    def _scroll_all(self, *args):
        self.numara_tree.yview(*args)
        for tree in self.trees.values():
            tree.yview(*args)

    def _create_edit_panel(self):
        edit_frame = ttk.Frame(self.right_panel)
        edit_frame.pack(fill=tk.X, pady=1, anchor="w")
        
        ttk.Label(edit_frame, text="Saat:", style="Bold.TLabel").grid(row=0, column=0, padx=1)
        self.ent_saat = ttk.Entry(edit_frame, width=6)
        self.ent_saat.grid(row=0, column=1, padx=1)
        
        ttk.Label(edit_frame, text="Tür:", style="Bold.TLabel").grid(row=0, column=2, padx=1)
        self.cmb_tur = ttk.Combobox(edit_frame, 
                                   values=["Öğrenci", "Öğretmen", "Teneffüs"],
                                   state="readonly",
                                   width=6)
        self.cmb_tur.grid(row=0, column=3, padx=1)
        
        ttk.Button(edit_frame,
                   text="➕ Ekle",
                   command=self.zil_ekle,
                   style="Success.TButton").grid(row=0, column=4, padx=2)
        
        warning_frame = ttk.Frame(edit_frame)
        warning_frame.grid(row=0, column=5, padx=1, sticky="w")
        ...
        toplu_ekle_frame = ttk.Frame(edit_frame)
        toplu_ekle_frame.grid(row=1, column=0, columnspan=6, sticky="nsew", pady=1)
        
        edit_frame.grid_columnconfigure(0, weight=1)
        edit_frame.grid_rowconfigure(1, weight=1)
        
        inner_frame = ttk.LabelFrame(
            toplu_ekle_frame,
            text="📥 TOPLU ZİL EKLEME",
            style="Header.TLabelframe"
        )
        inner_frame.pack(fill=tk.BOTH, expand=True, padx=1)
        
        ttk.Label(
            inner_frame,
            text="3 adet saat girişi gereklidir. \n (Öğrenci-Öğretmen-Teneffüs):\n Örnek: 08:30-08:35-09:00",
            font=("Segoe UI", 6),
            wraplength=0
        ).pack(fill=tk.BOTH, pady=1, padx=1)
        
        self.topluekle_text = scrolledtext.ScrolledText(
            inner_frame,
            height=2,
            wrap=tk.WORD,
            font=("Segoe UI", 9)
        )
        self.topluekle_text.pack(fill=tk.BOTH, expand=True, padx=1, pady=3)
        
        ttk.Button(
            inner_frame,
            text="🚀Toplu Ekle",
            command=self.zil_toplu_ekle_yeni,
            style="Success.TButton",
            width=15
        ).pack(pady=4)
        
        def update_wraplength(event):
            width = inner_frame.winfo_width() - 8
            inner_frame.winfo_children()[0].config(wraplength=max(70, width))
        
        inner_frame.bind("<Configure>", update_wraplength)
    ...
    def setup_event_listeners(self):
        ...
        # Scheduler'dan gelen çalma isteklerini dinle
        def handle_scheduler_request(command):
            # Log seviyesini INFO yaparak takip edelim
            logging.info(f"🔔 ZAMANLAYICI TETİKLENDİ: {command} -> GUI'ye iletiliyor...")
    ...
    def listeyi_guncelle(self, *args):
        logging.debug("listeyi_guncelle çağrıldı, argümanlar: %s", args)
        try:
            ziller = listele_zil()
            gruplanmis = {'ogrenci': [], 'ogretmen': [], 'teneffus': []}
            
            for zil in ziller:
                tur = zil[2].lower().replace("öğrenci", "ogrenci") \
                            .replace("öğretmen", "ogretmen") \
                            .replace("teneffüs", "teneffus")
                if tur in gruplanmis:
                    gruplanmis[tur].append(zil[1])
            ...
            for i in range(14):
                ...
                self.numara_tree.insert("", tk.END, values=(f"{i+1}.",), tags=(tag,))
                for tur in ['ogrenci', 'ogretmen', 'teneffus']:
                    try:
                        value = gruplanmis[tur][i] if i < len(gruplanmis[tur]) else ""
                    except IndexError:
                        value = ""
                    self.trees[tur].insert("", tk.END, values=(value,), tags=(tag,))
            
            # Seçili öğeleri sıfırla
            self.selected = {'ogrenci': set(), 'ogretmen': set(), 'teneffus': set()}
            
            # Treeview'ları zorla yenile
            for tree in self.trees.values():
                tree.update()
                tree.yview_moveto(0)  # Görünümü başa al
            self.numara_tree.update()
            
            # GUI'yi zorla yenile
            self.root.update_idletasks()
            
            logging.debug("Zil tablosu başarıyla güncellendi")
            self._update_status_with_style("✅ Zil tablosu güncellendi", "normal")
        except Exception as e:
            logging.error(f"listeyi_guncelle hatası: {str(e)}", exc_info=True)
    ...
    def zil_ekle(self):
        ...
        tur = self.cmb_tur.get().strip().lower()
        tur = tur.replace("öğrenci", "ogrenci") \
                .replace("öğretmen", "ogretmen") \
                .replace("teneffüs", "teneffus")
        
        if not saat or not tur:
            logging.error(f"Eksik giriş: saat={saat}, tur={tur}")
            messagebox.showerror("Hata", "Saat ve tür alanları boş bırakılamaz!")
            return
        
        if not re.match(r'^([0-1][0-9]|2[0-3]):[0-5][0-9]$', saat):
            logging.error(f"Geçersiz saat formatı: {saat}")
            messagebox.showerror("Hata", "Geçersiz saat formatı! Örnek: 14:00")
            return
        
        if tur not in ["ogrenci", "ogretmen", "teneffus"]:
            logging.error(f"Geçersiz zil türü: {tur}")
            messagebox.showerror("Hata", "Geçersiz zil türü!")
            return
        
        # job_id'yi Telegram ile uyumlu tut
        job_id = f"{tur}_{saat.replace(':', '')}"
        logging.debug(f"GUI: Zil ekleme denemesi: saat={saat}, tur={tur}, job_id={job_id}")
        success = ekle_zil(saat, tur, job_id)
        if success:
            logging.info(f"GUI: Zil eklendi: {saat}, {tur}, {job_id}")
            
            # +++ YENİ EKLENEN KISIM BAŞLANGIÇ +++
            try:
                from utils.scheduler import refresh_scheduler
                refresh_scheduler()  # Zamanlayıcıyı yenile
                logging.info("♻️ Zamanlayıcı yenilendi (tekli ekleme)")
                
            except Exception as e:
                logging.error(f"⚠️ Zamanlayıcı yenilenemedi: {str(e)}")
                messagebox.showerror(
                    "Hata", 
                    "Zil eklendi ama zamanlayıcı güncellenemedi!\n" 
            ...
            self.cmb_tur.set("")
            self.listeyi_guncelle()  # Tabloyu yenile
            self.root.update_idletasks()  # GUI'yi zorla yenile
        else:
            logging.error(f"GUI: Zil ekleme başarısız: {saat}, {tur}, job_id={job_id}")
            messagebox.showerror("Hata", "Bu zil zaten kayıtlı veya eklenemedi!")
        
    def zil_toplu_ekle_yeni(self):
        raw_text = self.topluekle_text.get("1.0", tk.END).strip()
        if not raw_text:
            logging.warning("Toplu ekleme verisi boş")
            messagebox.showwarning("Uyarı", "Lütfen toplu ekleme verisi girin!")
            return
        ...
        for satir_no, line in enumerate(lines, 1):
            line = line.strip()
            if not line:
                continue
            try:
                if line.count('-') != 2:
                    raise ValueError("Eksik '-' karakteri! Format: Saat-Saat-Saat")
                saatler = line.split('-')
                if len(saatler) != 3:
                    raise ValueError("Tam olarak 3 saat girmelisiniz")
                turler = ['ogrenci', 'ogretmen', 'teneffus']
                for i, saat in enumerate(saatler):
                    saat = saat.strip()
                    if not re.match(r'^([0-1][0-9]|2[0-3]):[0-5][0-9]$', saat):
                        raise ValueError(f"{turler[i].capitalize()} saati geçersiz: {saat}")
                    tur = turler[i]
                    job_id = f"{tur}_{saat.replace(':', '')}"
                    logging.debug(f"GUI: Toplu zil ekleme denemesi: saat={saat}, tur={tur}, job_id={job_id}")
                    if ekle_zil(saat, tur, job_id):
                        basarili += 1
                        logging.info(f"GUI: Toplu zil eklendi: {saat}, {tur}, {job_id}")
                        ...
                        try:
                            ...
                            if job_id not in job_ids:
                                logging.warning(f"Uyarı: {job_id} scheduler'da bulunamadı!")
                            else:
                                logging.info(f"Başarılı: {job_id} scheduler'da bulundu")
                            # Veritabanını kontrol et
                            with sqlite3.connect(DB_PATH) as conn:
                                cursor = conn.execute(
                                    "SELECT id, aktif FROM zil_saatleri WHERE job_id=?",
                                    (job_id,)
                                )
                                result = cursor.fetchone()
                                if result:
                                    logging.debug(f"Veritabanı: Zil ID={result[0]}, aktif={result[1]}")
                                else:
                                    logging.error(f"Veritabanında {job_id} bulunamadı!")
                        except Exception as e:
                            logging.error(f"Scheduler veya veritabanı kontrol hatası: {str(e)}", exc_info=True)
                    else:
                        hatali += 1
                        hatali_satirlar.append(f"{satir_no}. Satır: {tur} {saat} zaten kayıtlı veya eklenemedi")
                        logging.error(f"GUI: Toplu zil ekleme başarısız: {saat}, {tur}, {job_id}")
            except Exception as exc:
                hatali += 3
                hatali_satirlar.append(f"{satir_no}. Satır: {str(exc)}")
                logging.error(f"GUI: Toplu ekleme hatası, satır {satir_no}: {str(exc)}")
        ...
        if basarili > 0:
            ...
            try:
                from utils.scheduler import refresh_scheduler
                refresh_scheduler()  # Zamanlayıcıyı yenile
                logging.info("♻️ Zamanlayıcı yenilendi (toplu ekleme)")
                
            except Exception as e:
                logging.error(f"⚠️ Zamanlayıcı yenilenemedi: {str(e)}")
                messagebox.showerror(
                    "Hata", 
                    f"{basarili} zil eklendi ama zamanlayıcı güncellenemedi!\n"
                    "Lütfen uygulamayı yeniden başlatın."
                )
            # +++ YENİ EKLENEN KISIM BİTİŞ +++
        ...
        """
        if hatali_satirlar:
            mesaj += "\n\n🔍 Hata Detayları:\n" + "\n".join(hatali_satirlar)
        
        messagebox.showinfo("Toplu Ekleme Raporu", mesaj)
        self.listeyi_guncelle()  # Tabloyu yenile
        self.root.update_idletasks()  # GUI'yi zorla yenile
        self.topluekle_text.delete("1.0", tk.END)
