...
def get_duration(file_path):
    ...
    VLC kullanarak daha güvenilir şekilde süre hesaplar.
    """
    try:
        # Önce mutagen ile dene
        try:
            audio = MP3(file_path)
            return audio.info.length
        except Exception as e:
            logging.debug(f"Mutagen ile süre hesaplanamadı {file_path}: {str(e)}")
        ...
        try:
            ...
            if duration_ms > 0:
                return duration_ms / 1000.0  # milisaniyeden saniyeye
            else:
                # VLC süre alamazsa, varsayılan değer döndür
                logging.warning(f"VLC ile süre alınamadı {file_path}, varsayılan 30s kullanılıyor")
                return 30.0
                
        except Exception as e:
            logging.warning(f"VLC ile süre hesaplanamadı {file_path}: {str(e)}")
            return 30.0  # Varsayılan süre
            
    except Exception as e:
        logging.error(f"Süre hesaplama hatası {file_path}: {str(e)}")
        return 30.0  # Varsayılan süre
