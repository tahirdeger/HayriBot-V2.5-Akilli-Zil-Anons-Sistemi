...
class MediaManager:
    ...
    @property
    def loop(self):
        """Ana asyncio event loop'unu lazily döndürür."""
    ...
    def _call_threadsafe(self, func):
        """Event loop'a güvenli şekilde fonksiyon gönderir."""
        try:
            loop = self.loop
            if loop.is_running():
                loop.call_soon_threadsafe(func)
        except Exception as e:
            logging.debug(f"call_threadsafe hatası (normal olabilir): {e}")

    def __init__(self):
        if not getattr(self, '_initialized', False):  # Yalnızca bir kez başlat
            self._loop = None  # Lazy başlatılacak
            self.players = {}
    ...
    def _check_player_state(self, player, name):
        ...
        try:
            ...
            if not player.is_playing() and self.is_playing and self.currently_playing == name:
                ...
                self._call_threadsafe(lambda: ee.emit("media_ended", "state_check"))
        except Exception as e:
    ...
    def set_busy_status(self, is_busy, message=""):
        ...
        self._call_threadsafe(lambda: ee.emit("media_busy_status", {
            "is_busy": is_busy,
            "message": message
        }))
    ...
    def _on_media_ended(self, event):
        ...
        try:
            ...
            self._call_threadsafe(lambda: ee.emit("media_ended", event_type_str))
    ...
    def _original_play_media(self, name):
        ...
        try:
            ...
            # VLC'nin başlamasını bekle (max 3 saniye, 100ms aralıklı)
            _t0 = time.time()
            state = vlc.State.NothingSpecial
            ...
            if state not in (vlc.State.Playing, vlc.State.Buffering):
                logging.warning(f"⚠️ {name} play() sonrası başlamadı – state: {state}")
                player.stop()
                player.release()
                player_data['player'] = None
                return False
    ...
    def stop_all_media(self):
        ...
        try:
            ...
            self._call_threadsafe(lambda: ee.emit("media_cancelled"))
            self._call_threadsafe(lambda: ee.emit("media_ended", "forced_stop"))
            self._call_threadsafe(lambda: ee.emit("all_media_stopped"))
    ...
    def set_global_volume(self, volume_level):
        ...
        vol = self.global_volume * 100
        self._call_threadsafe(lambda: ee.emit("volume_changed", vol))
